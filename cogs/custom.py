...
class Custom(commands.Cog):
    ...
    async def fix(self, ctx):
        if ctx.author.id in league.keys():
            embed = disnake.Embed(
                description=f"[Click here](https://futbotleagues.leaguerepublic.com/matches/342846686/-1_-1/{league[ctx.author.id]}/-1/year2021_month010_day11.html) to see your League fixtures for today ```fix\nNote: site won't work or load if all of your fixes are done\nHeard Leagues have been stopped for the moment tho```",
    ...
    @commands.command(
        name="search",
        description="A command to find details of any FUT Player from the Player Database",
        usage="<playername>"
    )
    ...
    @commands.command(
        name="find",
        description="Find players and their stats from the CG Player Database (90+ only atm)",
        usage="<rating> <type>"
    )
